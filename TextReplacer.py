...
class TextReplacer:
    ...
    def __init__(self, presentation_file_name,
        ...
        self._presentation_file_name = self._ensure_unicode(
            presentation_file_name)
        if not os.path.exists(self._presentation_file_name):
            raise ValueError("Presentation file %s does not exist." %
        ...
        if len(slides.strip()) == 0:
            ...
            for rr in re.split('\\s*,\\s*', slides.strip()):
                ...
                if low is None or high is None:
                    raise ValueError(
                        'Slide list (--slides "%s") is not a comma separated list of slide numbers (i.e. 1) or slide number ranges (i.e. 4-12)' % (slides))
                if low < 1 or low > slide_cnt:
                    raise ValueError(
                        'Slide number %s in list (--slides "%s") is lower than 1 or bigger than the last slide number %s' % (low, slides, slide_cnt))
                if high < 1 or high > slide_cnt:
                    raise ValueError(
                        'Slide number %s in list (--slides "%s") is lower than 1 or bigger than the last slide number %s' % (high, slides, slide_cnt))
                if low > high:
                    raise ValueError(
                        'Slide range %s in list (--slides "%s") is invalid.' % (r, slides))
                for i in range(low-1, high):
                    self._slides[i] = True

    def replace_text(self, replacements, use_regex=False, verbose=None, quiet=None):
        self._replacements = list((self._ensure_unicode(
            srch), self._ensure_unicode(repl)) for (srch, repl) in replacements)
            ...
            for i in range(0, len(self._replacements)-1):
                ...
                for j in range(i+1, len(self._replacements)):
                    ...
                    if repl_i.find(srch_j) >= 0:
                        self._write_warning(
                            "Replacement string %s at index %s matches search string %s at index %s. This may produce unintended results due to chained replacements!" % (repl_i, i, srch_j, j))
                    if srch_j.find(srch_i) >= 0:
                        if srch_j.replace(srch_i, repl_i) == repl_j:
                            self._write_warning(
                                "Match/Replacement ('%s','%s') at index %s is obsolete due to match/replacement ('%s','%s') at index %s" % (srch_j, repl_j, j, srch_i, repl_i, i))
                        else:
                            self._write_warning(
                                "Match/Replacement ('%s','%s') at index %s will never match due to match/replacement ('%s','%s') at index %s" % (srch_j, repl_j, j, srch_i, repl_i, i))
        ...
        for slide in self._presentation.slides:
            if self._verbose:
                print("  Slide[%s, id=%s] with title '%s'" % (self._current_slide_idx+1, slide.slide_id,
                      "<no title>" if slide.shapes.title is None else slide.shapes.title.text))
        ...
        if len(self._messages) > 0:
            print(
                "The following warnings and errors have been issued during this run:", file=sys.stderr)
    ...
    def _replace_text_in_text_frame(self, level, shape, text_frame):
        ...
        for (srch, replacement) in self._replacements:
            text = "\n".join("".join(self._ensure_unicode(run.text)
            ...
            if pos_in_text_frame < 0:
                if self._verbose:
                    print("%sTrying to match '%s' -> no match" %
                          ("  "*level, srch))
            while pos_in_text_frame >= 0:
                ...
                for paragraph in text_frame.paragraphs:
                    para_text = "".join(self._ensure_unicode(run.text)
                    ...
                    if pos_in_paragraph >= paragraph_len:
                        ...
                        (to_match, to_replace) = self._replace_runs_text(
                            level+1, shape, paragraph_idx, paragraph.runs, pos_in_paragraph, to_match, to_replace)
    ...
    def _save_font_configuration(self, font):
        ...
        if font.color.type == MSO_COLOR_TYPE.SCHEME:
            ...
            saved['color.rgb'] = None if font.color.rgb is None else str(
                font.color.rgb)
            ...
            if pos >= olen:
                ...
                while i < cnt:
                    ...
                    if pos+match_len < olen:
                        ...
                        run.text = otext[0:pos] + \
                            to_replace+otext[pos+match_len:]
                    ...
                    to_match = to_match[part_match_len:]
    ...
    def _process_text_frame(self, level, shape, text_frame):
        if self._verbose:
            ...
            paragraph_idx = 0
            for paragraph in text_frame.paragraphs:
                print("%sParagraph[%s]: '%s'" % (
                    "  "*(level+1), paragraph_idx, self._make_printable(paragraph.text)))
                run_idx = 0
                for run in paragraph.runs:
                    print("%sRun[%s,%s]: '%s'" % (
                        "  "*(level+2), paragraph_idx, run_idx, self._make_printable(run.text)))
                    run_idx += 1
                paragraph_idx += 1
    ...
    def _process_shapes(self, level, shape_list_parent):
        for shape in shape_list_parent.shapes:
            if self._verbose:
                print("%sShape[%s, id=%s, type=%s]" % (
                    "  "*level, shape_list_parent.shapes.index(shape), shape.shape_id, shape.shape_type))
            ...
            if shape.has_chart:
                chart = shape.chart
                if self._verbose:
                    print("%sChart of type %s" %
                          ("  "*(level+1), chart.chart_type))
                if self._charts:
                    ...
                    for category in chart.plots[0].categories:
                        ...
                        for (srch, replace) in self._replacements:
                            if self._use_regex:
                                ...
                                changed_category = category.replace(
                                    srch, replace)
                            if changed_category == category:
                                if self._verbose:
                                    print("%sReplacing '%s' -> no match" %
                                          ("  "*(level+3), srch))
                            else:
                                if self._verbose:
                                    print("%sReplacing '%s' -> changed to '%s'" %
